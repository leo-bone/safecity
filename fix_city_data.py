#!/usr/bin/env python3
"""SafeCity v9.0 数据修复脚本 - 补充完整emergencyContacts和conflict数据"""
...
CITY_DATA_UPDATES = {
    ...
    "new_york": {
        "emergencyContacts": {
            ...
            "consulates": [
                {"name": "中国驻纽约总领事馆", "phone": "212-244-9392", "address": "520 12th Ave, New York, NY 10036", "hours": "周一至周五 9:00-12:00", "emergency": False, "note": "签证办理在领事馆大楼"},
                {"name": "驻纽约中国签证申请服务中心", "phone": "212-868-2078", "address": "90 John St, Suite 201, New York, NY 10038", "hours": "周一至周五 9:00-15:00", "emergency": False}
            ],
            "safetyApps": [
            ...
            "transport": {
                "modes": [
                    ...
                    {"icon": "🚌", "name": "公交", "description": "覆盖地铁不到的地区", "tips": ["下车需拉铃", "报站有延迟"]},
                    {"icon": "🚕", "name": "出租车/Uber", "description": "黄色出租车或Uber/Lyft", "tips": ["Uber更便宜更安全", "出租车必须打表"]}
                ]
            }
        },
        "conflict": {
            ...
            "civil_unrest": "低",
            "civil_desc": "政治集会偶有发生，通常和平。曼哈顿中城、联合广场可能有关门抗议，提前查看新闻。"
        },
        "food": [
            {"name": "热狗/ Pretzel", "type": "街头小吃", "price": "$3-8", "desc": "纽约街头经典美食", "foodSafety": "选择干净摊位，随身带湿纸巾"},
            {"name": "百吉饼 (Bagel)", "type": "早餐", "price": "$5-12", "desc": "配奶油奶酪和烟熏三文鱼", "tip": "去上东区或西村的正宗小店"},
            {"name": "芝士蛋糕 (Cheesecake)", "type": "甜点", "price": "$8-15", "desc": "纽约特产，口感浓郁", "tip": "Junior's或Eileen’s是经典"},
    ...
    "paris": {
        "emergencyContacts": {
            ...
            "selfProtection": [
                {"icon": "🎭", "title": "防盗要点", "tips": ["埃菲尔铁塔、卢浮宫周边假 petition 骗局", "地铁1号线假警察查证件是骗局", "蒙马特高地有人强卖手链"]},
            ...
            "transport": {
                "modes": [
                    ...
                    {"icon": "🚲", "name": "共享单车/Vélib", "description": "自行车共享系统", "tips": ["需法国手机号注册"]}
                ]
            }
        },
        "conflict": {
            ...
            "civil_unrest": "中等",
            "civil_desc": "黄背心运动曾大规模抗议，现在仍有零星罢工。出行前查好是否有游行，避开协和广场、凯旋门区域。"
        },
        "food": [
    ...
    "london": {
        ...
        "conflict": {
            ...
            "civil_unrest": "低",
            "civil_desc": "脱欧期间有抗议活动，总体平和。重大政治事件时唐宁街和白厅可能有示威，提前规划路线。"
        },
        "food": [
    ...
    "hong_kong": {
        "emergencyContacts": {
            ...
            "hospitals": [
                {"name": "玛丽医院", "phone": "2255-3838", "address": "Pokfulam, Hong Kong", "features": ["24H急诊", "英语服务", "大学教学医院"], "emergency24h": True},
                {"name": "伊利沙伯医院", "phone": "2958-8888", "address": "10 Granville Rd, Tsim Sha Tsui", "features": ["24H急诊", "语言粤语/英语/普通话"], "emergency24h": True},
                {"name": "养和医院", "phone": "2572-0211", "address": "2 Village Rd, Happy Valley", "features": ["24H急诊", "高端私立", "全英语服务"], "emergency24h": True}
            ],
            "consulates": [
                {"name": "中国外交部驻港公署", "phone": "2106-6888", "address": "Victoria Peak, Hong Kong", "hours": "周一至周五", "emergency": False},
                {"name": "各国驻港领事馆", "phone": "2829-3010", "address": "查询入境事务处网站", "hours": "工作日", "emergency": False}
            ],
            "safetyApps": [
                {"name": "1823", "icon": "📱", "description": "香港政府热线App"},
                {"name": "MTR Mobile", "icon": "📱", "description": "港铁实时到站信息"}
            ],
            "selfProtection": [
                {"icon": "🔒", "title": "财产安全", "tips": ["购物时信用卡遮挡", "油尖旺地区警惕找换店假币", "夜市人多看好包"]},
                {"icon": "🌃", "title": "夜生活安全", "tips": ["兰桂坊酒吧区有醉酒者", "避免与陌生人拼桌", "注意计程车计价器"]},
                {"icon": "🗺️", "title": "区域注意", "tips": ["重庆大厦背包客天堂但人员复杂", "深水埗电子产品店注意价格欺诈"]}
            ],
            "transport": {
                "modes": [
                    {"icon": "🚇", "name": "港铁MTR", "description": "覆盖全面，干净准点", "tips": ["机场快线贵但快", "八达通卡通用所有交通"]},
                    {"icon": "🚌", "name": "巴士/小巴", "description": "路线覆盖全港", "tips": ["绿色小巴按铃下车", "红色小巴不报站"]},
                    {"icon": "🚕", "name": "的士", "description": "红(市区)、绿(新界)、蓝(大屿山)", "tips": ["过海隧道费乘客付", "后备箱放行李另加费"]}
                ]
            }
        },
        "conflict": {
        ...
        "food": [
            {"name": "港式茶餐厅", "type": "本地", "price": "HK$40-80", "desc": "菠萝油、丝袜奶茶、西多士", "tip": "早餐套餐最划算"},
            {"name": "点心", "type": "粤菜", "price": "HK$60-150", "desc": "虾饺、烧麦、叉烧包", "foodSafety": "连锁店出品稳定"},
            {"name": "鱼蛋/咖喱鱼丸", "type": "街头小吃", "price": "HK$20-35", "desc": "旺角街头经典", "tip": "加辣更好吃"},
            {"name": "烧腊", "type": "粤菜", "price": "HK$60-120", "desc": "叉烧、烧鹅、油鸡", "tip": "一乐烧鹅是米其林推荐"}
        ],
        "festivals": [
            {"name": "农历新年", "date": "1月底-2月", "desc": "花市、烟花、舞狮"},
            {"name": "香港电影节", "date": "4月", "desc": "国际电影展映"},
            {"name": "佛诞节", "date": "5月", "desc": "大屿山宝莲寺浴佛"},
            {"name": "香港回归纪念日", "date": "7月1日", "desc": "烟花汇演"}
        ],
        "customs": [
            "香港左舵右行(靠左行)",
            "地铁/公交禁止饮食",
...
def upgrade_city_data(content, city_id, city_data):
    """为指定城市升级数据"""
    # 找到城市开始位置
    city_pattern = rf'("{city_id}"\s*:\s*\{{)'
    ...
    # 找到lifestyle部分
    lifestyle_pattern = rf'("{city_id}".*?"lifestyle"\s*:\s*\{{)'
    lifestyle_match = re.search(lifestyle_pattern, content[start_pos:])
    if not lifestyle_match:
        return content, False
    
    # 找到lifestyle结束位置(匹配花括号)
    lifestyle_start = start_pos + lifestyle_match.end() - 1
    lifestyle_end = lifestyle_start
    brace_count = 1
    in_string = False
    i = lifestyle_end + 1
    ...
    # 构建新的lifestyle数据
    new_lifestyle = {
        "emergencyContacts": city_data.get("emergencyContacts", {}),
        "food": city_data.get("food", []),
        "festivals": city_data.get("festivals", []),
        "customs": city_data.get("customs", []),
        "transport": city_data.get("transport", {})
    }
    
    # 如果原lifestyle有其他数据保留
    old_lifestyle_start = lifestyle_start
    old_lifestyle_end = lifestyle_end
    
    # 替换lifestyle部分
    new_content = (
        content[:old_lifestyle_start] +
        "\n      \"emergencyContacts\": " + format_json(city_data.get("emergencyContacts", {})) + "," +
        "\n      \"food\": " + format_json(city_data.get("food", [])) + "," +
        "\n      \"festivals\": " + format_json(city_data.get("festivals", [])) + "," +
        "\n      \"customs\": " + format_json(city_data.get("customs", [])) +
        "\n      \"transport\": " + format_json(city_data.get("transport", {})) +
        content[old_lifestyle_end:]
    )
    
    # 添加conflict数据(在overview字段之前)
    if "conflict" in city_data:
        overview_pattern = rf'("{city_id}".*?"overview"\s*:\s*")'
        overview_match = re.search(overview_pattern, new_content)
        if overview_match:
            conflict_data = '\n    "conflict": ' + format_json(city_data["conflict"]) + ','
            pos = overview_match.start()
            new_content = new_content[:pos] + conflict_data + new_content[pos:]
...
def main():
    ...
    with open('data.js.backup', 'w', encoding='utf-8') as f:
        f.write(content)
    print("已备份原文件到 data.js.backup")
    
    updated_cities = []
    failed_cities = []
    
    for city_id, city_data in CITY_DATA_UPDATES.items():
        new_content, success = upgrade_city_data(content, city_id, city_data)
        if success:
            content = new_content
            updated_cities.append(city_id)
            print(f"✓ 已更新 {city_id}")
        else:
    ...
    print(f"\n更新完成: {len(updated_cities)} 个城市")
    if failed_cities:
        print(f"失败: {failed_cities}")
